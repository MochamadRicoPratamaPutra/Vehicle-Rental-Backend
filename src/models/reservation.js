  ...
  return new Promise((resolve, reject) => {
    if (column !== undefined && sort !== undefined && keyword !== undefined && search !== undefined) {
      if (Number.isNaN(page) === false && Number.isNaN(limit) === false) {
        ...
        connection.query(`SELECT * FROM reservation WHERE ${search} LIKE '%${keyword}%' ORDER BY ${column} ${sort} LIMIT ${limit} OFFSET ${startIndex};`, (error, result) => {
          if (!error) {
      ...
      if (Number.isNaN(page) === false && Number.isNaN(limit) === false) {
        ...
        const endIndex = page * limit
        const paginatingResult = {}
        connection.query(`SELECT * FROM reservation ORDER BY ${column} ${sort} LIMIT ${limit} OFFSET ${startIndex}`, (error, result) => {
          if (!error) {
            connection.query('SELECT COUNT(*) AS count FROM reservation', (errorCount, resultCount) => {
              if (!errorCount) {
                console.log(resultCount[0].count)
      ...
      if (Number.isNaN(page) === false && Number.isNaN(limit) === false) {
        ...
        const endIndex = page * limit
        const paginatingResult = {}
        connection.query(`SELECT * FROM reservation WHERE ${search} LIKE '%${keyword}%' LIMIT ${limit} OFFSET ${startIndex}`, (error, result) => {
          if (!error) {
            connection.query('SELECT COUNT(*) AS count FROM reservation', (errorCount, resultCount) => {
              if (!errorCount) {
                console.log(resultCount[0].count)
...
const getReservationById = (id) => {
  return new Promise((resolve, reject) => {
    connection.query('SELECT * FROM reservation WHERE id = ?', id, (error, result) => {
      if (!error) {
        resolve(result)
      } else {
        reject(error)
